          ...
          {tableName === 'locations' && (
            <>
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
                 <TextField select size="small" label="年" value={filterYear || ''} onChange={(e) => fetchTableData(tableName, { filterYear: e.target.value, filterMonth: '', filterDay: '' })} sx={{ width: 100 }}>{availableYears.map(y => <MenuItem key={y} value={y}>{y}</MenuItem>)}</TextField>
                 <TextField select size="small" label="月" value={filterMonth || ''} onChange={(e) => fetchTableData(tableName, { filterMonth: e.target.value, filterDay: '' })} sx={{ width: 80 }}>{availableMonths.map(m => <MenuItem key={m} value={m}>{m}</MenuItem>)}</TextField>
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                    ...
                    <TimePicker label="结束" ampm={false} minutesStep={1} value={endTime ? dayjs(endTime, 'HH:mm:ss') : null} onChange={(v: any) => fetchTableData(tableName, { endTime: v?.format('HH:mm:ss') })} slotProps={{ textField: { size: 'small' } }} />
                </Box>
